...
def main(descargar=True):
    ...
    for n, c in ngramas4_lib.items():
        final4[n] = final4.get(n, 0) + c
    print(f"  {len(final4)} 4-gramas únicos totales")

    print("  Construyendo tabla ngramas4...")
    conn = sqlite3.connect(RUTA_DB)
    conn.execute("""CREATE TABLE ngramas4 (
        w1 TEXT, w2 TEXT, w3 TEXT, w4 TEXT, freq INTEGER DEFAULT 1
    )""")
    batch = []
    conn.execute("BEGIN")
    idx = 0
    for (w1, w2, w3, w4), freq in final4.items():
        batch.append((w1, w2, w3, w4, freq))
        idx += 1
        if len(batch) >= 200000:
            conn.executemany(
            ...
            batch = []
            if idx % 1000000 == 0:
                print(f"    {idx//1000000}M insertados...", end='\r')
    if batch:
        conn.executemany(
            "INSERT INTO ngramas4 (w1, w2, w3, w4, freq) VALUES (?, ?, ?, ?, ?)", batch)
    conn.commit()
